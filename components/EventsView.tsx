...
import { Plus, Calendar, MapPin, Users, Music, Tag, Check, ChevronRight } from 'lucide-react';
...
interface EventsViewProps {
  ...
  onCreate: (event: Omit<Event, 'id' | 'status'>) => void;
}

const EventsView: React.FC<EventsViewProps> = ({ events, activeEventId, onSelect, onCreate }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onCreate(formData);
    setShowForm(false);
    setFormData({ name: '', date: '', location: '', capacity: 0, category: 'Concert' });
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 text-sm">Créez et activez vos sessions de contrôle</p>
        </div>
        <button 
      ...
      {showForm && (
        <div className="bg-slate-900 border border-emerald-500/30 p-6 rounded-3xl shadow-2xl animate-in zoom-in duration-300">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Capacité Attendue</label>
              <div className="relative">
            ...
            <div className="md:col-span-2">
              ...
              >
                <Check className="w-5 h-5" /> ENREGISTRER L'ÉVÉNEMENT
              </button>
            </div>
          </form>
        </div>
        ...
        {events.map((event) => (
          ...
          >
            ...
            <div className="space-y-2 mt-4">
              ...
              <div className="flex items-center gap-2 text-slate-400 text-xs">
                <Users className="w-3.5 h-3.5" /> {event.capacity.toLocaleString()} personnes
              </div>
            </div>
